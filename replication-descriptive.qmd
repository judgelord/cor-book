...
here::here("data", "member_data.Rdata")|> load()


here::here("data", "all_contacts.rda") |> str_replace("cor-book", "correspondence_data") |> load()

here::here("data", "dcounts-min.rda") |> str_replace("cor-book", "correspondence_data") |> load()
...
congress_from_year <- function(year) {
  floor((year - 1789) / 2) + 1
}

 dcounts %<>% 
  mutate(congress =  congress_from_year(year)) %>% #distinct(congress)
  left_join(member_data |> select(member_state, bioname, icpsr, chamber, congress,cqlabel, pop2010, party))

percentiles <- dcounts %>% 
  dplyr::filter(year > 2006, year < 2021) %>% 
  dplyr::group_by(member_state, chamber, year, party) %>% 
  dplyr::summarise(n = sum(per_icpsr_chamber_year_agency_type)) %>%
  dplyr::group_by(member_state, chamber, party) %>% 
  ...
  dplyr::select(member_state, Percentile, mean, chamber, rank, party) %>% 
  dplyr::distinct() 
...
dcounts_per_legislator <- dcounts  %>% filter(agency != "DHS_USCIS") %>%
...
ggsave(here::here("figs", "districts", paste0(cq, ".png")), height = 2.5, width = 5.7)
} 
```
